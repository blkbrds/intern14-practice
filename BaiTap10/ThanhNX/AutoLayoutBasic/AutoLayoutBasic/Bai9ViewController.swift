...
final class Bai9ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        infoLabel.text = infoPlanet[0]
    }
    
    @IBAction private func nextButtonTouchUpInside() {
        if imageScrollView.contentOffset.x >= imageScrollView.contentSize.width - 300 {
            imageScrollView.contentOffset.x = 0
            index = 0
        } else {
            imageScrollView.contentOffset.x = imageScrollView.contentOffset.x + 300
            index += 1
        }
        infoLabel.text = infoPlanet[index]
    }
    
    @IBAction private func backButtonTouchUpInside() {
        if imageScrollView.contentOffset.x < 300 {
            index = 9
            imageScrollView.contentOffset.x = 2700
        } else {
            index -= 1
            imageScrollView.contentOffset.x = imageScrollView.contentOffset.x - 300
        }
